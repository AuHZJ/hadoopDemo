...
public class SortByDegree extends Configured implements Tool {
	...
	public int run(String[] arg0) throws Exception {
		...
		Job job = Job.getInstance(conf, "SortByDegree");
		job.setJarByClass(SortByDegree.class);
		job.setMapperClass(SBDMapper.class);
		job.setMapOutputKeyClass(TagDegree.class);
		job.setMapOutputValueClass(NullWritable.class);
		job.setInputFormatClass(TextInputFormat.class);
		job.setOutputFormatClass(TextOutputFormat.class);
		TextInputFormat.addInputPath(job, new Path("/knn_data/gsd_result/part-r-00000"));
